...
//dirty func to manage prestarted apps
void CatchTopApplicationMessagesFix()
{
 		
	         DBusConnection *buss;
	         DBusError errorr;

	         fixloop = g_main_loop_new (NULL, FALSE);

	         dbus_error_init (&errorr);
	         buss = dbus_bus_get (DBUS_BUS_SESSION, &errorr);
	         if (!buss) {
	           g_warning ("Failed to connect to the D-BUS daemon: %s", errorr.message);
	           dbus_error_free (&errorr);
	           return 1;
	         }
	         dbus_connection_setup_with_g_main (buss, NULL);

	         /* listening to messages from all objects as no path is specified */
	         dbus_bus_add_match (buss, "type='method_call', member='top_application'",&errorr);
	         dbus_connection_add_filter (buss, TopApplicationMessageHandler, loopp, NULL);

	         g_main_loop_run (loopp);
	         return 0;
}

static DBusHandlerResult TopApplicationMessageHandler (DBusConnection *connection, DBusMessage *message, void *user_data)
{
